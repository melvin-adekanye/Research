...
def is_2p2_free(G):
    ...
    for node in G.nodes:
        ...
        # Check if any pair of neighbors is connected by an edge
        for i in range(len(neighbors)):
            for j in range(i+1, len(neighbors)):
                if G.has_edge(neighbors[i], neighbors[j]):
                    # If an edge exists between the neighbors, the graph is not 2P2-free
                    return False
    # If no pair of neighbors is connected by an edge, the graph is 2P2-free
    return True
