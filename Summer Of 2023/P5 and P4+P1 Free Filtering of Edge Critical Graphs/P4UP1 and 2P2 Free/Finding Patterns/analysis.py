...
for (index, string) in enumerate(GRPAH6_STRING):
    ...
    degresss = graph.degree()

    diff_in_degreee = abs(min(degresss) - max(degresss))

    print(
        f"Degrees(max-min) = {diff_in_degreee} ---> {graph.graph6_string()} {'     ---> Compliment(k3 + 2p2)-free' if diff_in_degreee == 0 or diff_in_degreee == 2 else '' }")
