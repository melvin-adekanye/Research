...
// M+C June 26, 2023
static int test_if_contains_induced_p4_p1 = 0;
...
// M+C - June 26, 2023
static unsigned long long int times_graph_p4_p1[MAXN + 1] = {0};
static unsigned long long int times_graph_not_p4_p1[MAXN + 1] = {0};
...
static void complete_dest_arr(char dest_arr[], int range)
{
...
static void arr_diff(const char one[], const char two[], int inLength, char outArr[], int *outLength)
{
...
static void compliment_of_current(GRAPH g)
{
    for (int currentNode = 0; currentNode < nv; currentNode++) {

        char newRow[nv];
        int newRowLen;

        complete_dest_arr(newRow, nv);
        arr_diff(current_graph[currentNode], newRow, nv, newRow, &newRowLen);

        int index = 0;
        for (int j = 0; j < newRowLen; j++) {

            if (newRow[j] != currentNode) {
                g[currentNode][index] = newRow[j];
                index++;
            }
...
output_graph(GRAPH g, ADJACENCY adj, int num_vertices)
{
    ...
    compliment_of_current(compliment_graph);
    ...
    if (fwrite(codebuffer, sizeof(unsigned char), codelength, stdout) < sizeof(unsigned char) * codelength)
    {
...
compute_colour_domination_graphs_vertex_removed()
{
    ...
    {
        if (!ISMARKED2(removed_vertex))
        { // If already marked then was already determined!
                ...
                unsigned int forbidden[], int *nofc, unsigned int *MASK, int genug, int *good_enough)
{
    ...
    for (i = 1; colours[i]; i++)
        ; // Now must start from 1, since first vertex may not be coloured already by chromatic_number_same_colour_vertices()
    maxforbidden = nofc[i];
    vertex = i;
    ...
    for (colour = 1; (colour <= usedsofar + 1) && (colour < *minsofar) && (usedsofar < *minsofar) && !(*good_enough); colour++)
    {
...
int chromatic_number(GRAPH graph, ADJACENCY adj, int genug)
{
    ...
    if (genug && (genug < minsofar))
        minsofar = genug + 1; /* Es wird einfach so getan, als haette man schon
so eine Faerbung (wuerde ja eh nichts nuetzen */

    if (minsofar >= 32)
    {
        printf("Too many possible colours for the colouring function\n");
        exit(1);
    }
...
so eine Faerbung (wuerde ja eh nichts nuetzen */
    ...
    for (i = 1; i <= graph[0][0]; i++)
    {
        if (ISELEMENT1(&same_colour_vertices, i - 1))
        {
            ...
            {
                if (!(MASK[1] & forbidden[nachbar]))
                { // Since same colour vertices may have same neighbour
                    nofc[nachbar]++;
                    forbidden[nachbar] |= MASK[1];
                }
            }
...
so eine Faerbung (wuerde ja eh nichts nuetzen */
    ...
    for (i = 0; i < vertices_size; i++)
    {
        ...
        {
            if (!(MASK[given_colours[i]] & forbidden[nachbar]))
            { // Since same colour vertices may have same neighbour
                nofc[nachbar]++;
...
determine_longest_disjoint_induced_path_recursion(int maxlength)
{
    ...
    for (i = 0; i < adj[previous_vertex]; i++)
    {
        ...
        if (!ISELEMENT1(&current_path_with_forbidden_neighbourhood, neighbour))
...
contains_forbidden_induced_cycle_recursion(int forbiddencyclesize)
{
    ...
    for (i = 0; i < adj[previous_vertex]; i++)
    {
        ...
        if (ISELEMENT1(&possible_cycle_vertices, next_vertex))
        { // marks seem to be faster than bv here
                ...
                && (*GRAPHROW1(nautyg, next_vertex, MAXM) & current_path_bitvector) == BITT[previous_vertex])
            {
                ...
                DELELEMENT1(&current_path_bitvector, next_vertex);
                     ...
                     (*GRAPHROW1(nautyg, next_vertex, MAXM) & current_path_bitvector) == (BITT[previous_vertex] | BITT[current_path[0]]))
            {
    ...
    for (i = 0; i < adj[previous_vertex]; i++)
    {
        ...
        { // marks seem to be faster than bv here
                     ...
                     (*GRAPHROW1(nautyg, next_vertex, MAXM) & current_path_bitvector) == (BITT[previous_vertex] | BITT[current_path[0]]))
            {
                ...
                if (is_valid_crown(current_path_bitvector, forbiddencyclesize) && !cycle_was_already_stored(current_path_bitvector))
                {
        ...
        for (j = 0; j < adj[a]; j++)
        {
            ...
            for (k = 0; k < adj[b]; k++)
            {
                ...
                if (c != a && !ISELEMENT1(&nautyg[a], c))
                {
                    ...
                    {
                        d = current_graph[c][i];
                        if (d != b && !ISELEMENT1(&nautyg[b], d) && !ISELEMENT1(&nautyg[a], d))
                        {
                                                    ...
                                                    for (l = 0; l < adj[e]; l++)
                                                    {
                                                        ...
                                                        if (!ISELEMENT1(&forbidden_vertices, f))
                                                            for (m = 0; m < adj[f]; m++)
                                                            {
                                                                ...
                                                                if (g != e && !ISELEMENT1(&nautyg[g], e) && !ISELEMENT1(&forbidden_vertices, g))
                                                                {

                                                                    for (n = 0; n < adj[g]; n++)
                                                                    {
                                                                        h = current_graph[g][n];
                                                                        if (h != f && !ISELEMENT1(&nautyg[f], h) && !ISELEMENT1(&nautyg[e], h) && !ISELEMENT1(&forbidden_vertices, h))
                                                                        {
                                                                            forbidden_nbrs2 = 0;
                                                                            ...
                                                                            for (q = 0; q < adj[h]; q++)
                                                                            {
                                                                                ...
                                                                                if (!ISELEMENT1(&forbidden_vertices, e1) && (nautyg[e1] & forbidden_nbrs2) == 0)
                                                                                {
                                                                                    ...
                                                                                    for (r = 0; r < adj[e1]; r++)
                                                                                    {
                                                                                        ...
                                                                                        if (e < e2 && !ISELEMENT1(&forbidden_vertices, e2) && (nautyg[e2] & forbidden_nbrs2) == 0)
                                                                                        {
        ...
        for (j = 0; j < adj[a]; j++)
        {
            ...
            for (k = 0; k < adj[b]; k++)
            {
                ...
                if (c != a && !ISELEMENT1(&nautyg[a], c))
                {
                    ...
                    {
                        d = current_graph[c][i];
                        if (d != b && a < d && !ISELEMENT1(&nautyg[b], d) && !ISELEMENT1(&nautyg[a], d))
                        {
                            // Induced P4 found
                                        ...
                                        if (!ISELEMENT1(&forbidden_vertices, f))
                                            for (m = 0; m < adj[f]; m++)
                                            {
                                                ...
                                                if (g != e && !ISELEMENT1(&nautyg[g], e) && !ISELEMENT1(&forbidden_vertices, g))
                                                {
                                                    ...
                                                    {
                                                        h = current_graph[g][n];
                                                        if (h != f && e < h && !ISELEMENT1(&nautyg[f], h) && !ISELEMENT1(&nautyg[e], h) && !ISELEMENT1(&forbidden_vertices, h))
                                                        {
                                                            // Induced C4 found
    ...
    for (i = 0; i < nv - 3; i++)
    {
        for (j = 0; j < adj[i]; j++)
        {
            ...
            if (i < next)
            {
                ...
                {
                    if (k != next)
                    { // k != i is automatically implied because k >= i+1
                        for (l = 0; l < adj[k]; l++)
                        {
    ...
    for (i = 0; i < nv; i++)
        for (j = 0; j < nv; j++)
            if (i != j && NBH_IS_SUBSET_OF(i, j))
            {
                ...
                else if (adj[i] == degree_current_best_small && adj[j] > degree_current_best_large)
...
static void remove_edges_in_all_possible_ways(int edge_index, int num_edges_removed)
{
    ...
    if (num_edges_removed > 0)
    {
        ...
        else
        {
            ...
            if ((!test_if_contains_forbidden_cycle || !contains_forbidden_induced_cycle_all(forbidden_cycle_size)) && !contains_forbidden_induced_path())
            {
                is_critical = 0;
                return;
            }
    ...
    for (i = 0; i < nv; i++)
    {
        ...
        while ((neighbour_b1 = nextelement1(gv, 1, neighbour_b1)) >= 0)
        {
            ...
            while ((neighbour_b2 = nextelement1(&intersect_neigh_n_b1, 1, neighbour_b2)) >= 0)
            {
                ...
                if (triangle_combination_leads_to_bull(i, neighbour_b1, neighbour_b2) || triangle_combination_leads_to_bull(i, neighbour_b2, neighbour_b1) || triangle_combination_leads_to_bull(neighbour_b1, neighbour_b2, i))
                    return 1;
            }
        }
    }
...
static int contains_induced_fork()
{
    ...
    for (centervertex = 0; centervertex < nv; centervertex++)
        if (adj[centervertex] > 3)
        { // Centervertex has degree >= 4
...
test_if_combination_yields_gem(TRIANGLE triangle, int central_vertex, int central_vertex_index)
{
    ...
    while ((extra_triangle_vertex_edge0 = nextelement1(&intersect_neigh_edge0, 1, extra_triangle_vertex_edge0)) >= 0)
        if (extra_triangle_vertex_edge0 != edge0_third_vertex && !ISELEMENT1(GRAPHROW1(nautyg, edge0_third_vertex, MAXM), extra_triangle_vertex_edge0))
        {
            ...
            while ((extra_triangle_vertex_edge1 = nextelement1(&intersect_neigh_edge1, 1, extra_triangle_vertex_edge1)) >= 0)
                if (extra_triangle_vertex_edge1 != edge1_third_vertex && !ISELEMENT1(GRAPHROW1(nautyg, edge1_third_vertex, MAXM), extra_triangle_vertex_edge1) && !ISELEMENT1(GRAPHROW1(nautyg, extra_triangle_vertex_edge0, MAXM), extra_triangle_vertex_edge1))
                {
    ...
    for (i = 0; i < nv; i++)
    {
        ...
        while ((neighbour_b1 = nextelement1(gv, 1, neighbour_b1)) >= 0)
        {
            ...
            while ((neighbour_b2 = nextelement1(&intersect_neigh_i_b1, 1, neighbour_b2)) >= 0)
            {
                ...
                if (test_if_combination_yields_gem(triangle, triangle[0], 0) || test_if_combination_yields_gem(triangle, triangle[1], 1) || test_if_combination_yields_gem(triangle, triangle[2], 2))
                    return 1; // Induced gem found
            }
        }
    }
       ...
       int max_colours_nbh_colouring, unsigned char destroyed_small_vertex, int perform_extensions)
{
    ...
    if (!test_similar_element_destroyed_first)
    { // For P6-free graphs, it is faster to test this before P-free test
        if (!similar_element_is_destroyed(isolated_vertices, avoided_vertex))
        {
            times_similar_element_not_destroyed[nv]++;
            previous_extension_was_not_colourable = 1;
    ...
    if (max_colours_nbh_colouring > 0)
    {
        ...
        if (max_colours_in_valid_neighbourhood_colouring(destroyed_small_vertex, max_colours_nbh_colouring + 1) <= max_colours_nbh_colouring)
        {
    ...
    */
    ...
    if (!perform_extensions)
    {
        ...
        if (splay_lookup(&list_of_worklists[nv], can_form, &is_new_node) == NULL)
        { // i.e. is new node
            // Must also count uncol ones if returning when number_of_graphs_generated_from_parent == 0
            if (is_col)
                number_of_graphs_generated_from_parent++; // Number of non-iso P-free col graphs generated
    ...
    if (!test_girth && contains_similar_vertices(&best_small_similar_vertex, &other_larger_similar_vertex))
    {
        ...
        if (nv >= LEVEL_LEAST_CHILDREN && (nv % LEAST_CHILDREN_LEVEL_MOD) == 0 && num_similar_vertices > 1)
        {
            int i;
            int least_num_generated_from_parent = INT_MAX;
    ...
    else
    {
        ...
        else
        {
            ...
            if (contains_too_small_vertex(&best_small_vertex))
            {
                ...
                if (nv >= LEVEL_LEAST_CHILDREN && (nv % LEAST_CHILDREN_LEVEL_MOD) == 0 && num_small_vertices > 1)
                {
                    int i;
                    int least_num_generated_from_parent = INT_MAX;
                    for (i = 0; i < num_small_vertices && least_num_generated_from_parent > 0; i++)
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (contains_similar_edgepair(similar_edgepair, 0))
                    {
                        ...
                        if (nv >= LEVEL_LEAST_CHILDREN && (nv % LEAST_CHILDREN_LEVEL_MOD) == 0 && num_similar_edges > 1)
                        {
                            int i;
                            int least_num_generated_from_parent = INT_MAX;
                            int best_sim_pair = 0;
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (!test_if_trianglefree && !test_if_diamondfree && contains_similar_diamond(similar_diamondpair, 0))
                                        {
                                        ...
                                        else
                                        {
                                            ...
                                            if (num_colours == 3 && contains_odd_crown(&crown_vertices))
                                            {
                                                ...
                                                if (nv >= LEVEL_LEAST_CHILDREN && (nv % LEAST_CHILDREN_LEVEL_MOD) == 0 && num_cycles_stored > 1)
                                                {
                                                    int i;
                                                    int least_num_generated_from_parent = INT_MAX;
...
make_antihole(int size)
{
    ...
    for (i = 0; i < size; i++)
    {
        adj[i] = 0;
        for (j = 0; j < size; j++)
            if (j != i && j != ((i + size - 1) % size) && j != ((i + 1) % size))
                current_graph[i][adj[i]++] = j;
    }
}
...
init_construction()
{
    ...
    if (!test_if_trianglefree)
    {
        ...
        for (i = 7; i <= largest_antihole_size; i += 2)
        {
            ...
            if (!(test_if_contains_forbidden_cycle && contains_forbidden_induced_cycle(forbidden_cycle_size)))
                extend(isolated_vertices, 0, 0, 0, 1);
            else
