    ...
    if chromatic_number == GRAPH_K:
        ...
        for original_graph_and_index in original_graphs:
            ...
            process = multiprocessing.Process(
                target=multiprocess_critical_check, args=(original_graph_and_index, flag))
            process.start()
...
>>>>>>> master
    def foo(bar, result, index, event):
        ...
        result[index] = "foo"
        # Check if foo() returns a value
        if result[index] == "foo":

            print('set')

            # Set the event flag to signal thread termination
            event.set()
...
>>>>>>> master
    ...
    # Wait for the event flag to be set, indicating thread termination
    event.wait()
    print('kill')
